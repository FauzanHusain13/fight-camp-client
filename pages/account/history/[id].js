...
const DetailHistory = ({ confirmationDetail, user }) => {
    return (
        ...
        <div className="container px-3 md:px-5 lg:px-20 text-white m-auto">
            <h1 className="text-2xl md:text-3xl font-semibold tracking-wide">Detail History</h1>
            {confirmationDetail.map((item) => {
                return (
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
                            <div>
                                <div className="flex justify-between">
                                    <div>
                                        <h1 className="font-semibold text-xl md:text-2xl tracking-wide">{item.historyTraining.trainingName}</h1>
                                        <div className="flex">
                                    ...
                                    <div>
                                        <h1 className="bg-red-600 cursor-pointer p-2 font-semibold rounded-md text-sm">{item.historyTraining.package}</h1>
                        ...
                        <div className="mt-14">
                        {item.status === "pending" && (
                            <h1 className="text-sm text-center cursor-pointer w-full bg-orange-500 hover:bg-orange-600 transition-all duration-400 p-3 font-semibold">
                                Pending
                            </h1>
                        )}
                        {item.status === "success" && (
                            <h1 className="text-sm text-center cursor-pointer w-full bg-green-700 hover:bg-green-800 transition-all duration-400 p-3 font-semibold">
                                Success
                            </h1>
                        )}
                        {item.status === "failed" && (
                            <h1 className="text-sm text-center cursor-pointer w-full bg-red-700 hover:bg-red-800 transition-all duration-400 p-3 font-semibold">
                                Failed
                            </h1>
                        )}
                        </div>
